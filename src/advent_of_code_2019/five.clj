...
(defn get-input-values
  [param-values fn-type remaining-inputs]
  (case fn-type
       ...
       (if (= instruction 99)
         ...
         (let [opcode         (mod instruction 100)
               ...
               input-values   (get-input-values param-values
                                                type
                                                remaining-inputs)
               output-pointer (if (contains? #{:standard :input} type)
                                (last param-pointers))
               ...
               new-outputs    (if (= type :output)
                                ...
                                outputs)]
           (recur new-code new-pointer new-inputs new-outputs)))))))
